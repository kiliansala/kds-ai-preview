...
import {
  FIGMA_BUTTON_CONTRACT
} from '../.figma/button.figma-contract.js';
...
function validateComponent(componentPath: string): ValidationResult {
  ...
  if (!componentProps.size) {
    ...
    if (componentProps.size.default !== FIGMA_BUTTON_CONTRACT.properties.size.default) {
      result.errors.push(
        `size default value mismatch. Expected: "${FIGMA_BUTTON_CONTRACT.properties.size.default}", Got: "${componentProps.size.default}"`
      );
      result.valid = false;
    }

    // Validate allowed values
    const contractSizes = FIGMA_BUTTON_CONTRACT.properties.size.values as readonly string[];
  ...
  if (!componentProps.hierarchy) {
    ...
    if (componentProps.hierarchy.default !== FIGMA_BUTTON_CONTRACT.properties.hierarchy.default) {
      result.errors.push(
        `hierarchy default value mismatch. Expected: "${FIGMA_BUTTON_CONTRACT.properties.hierarchy.default}", Got: "${componentProps.hierarchy.default}"`
      );
      result.valid = false;
    }

    // Validate allowed values
    const contractHierarchies = FIGMA_BUTTON_CONTRACT.properties.hierarchy.values as readonly string[];
    const componentHierarchies = componentProps._buttonHierarchyValues || [];

    const missingHierarchies = contractHierarchies.filter((h: string) => !componentHierarchies.includes(h));
    const extraHierarchies = componentHierarchies.filter((h: string) => !contractHierarchies.includes(h));
  ...
  if (!componentProps.iconPosition) {
    ...
    if (componentProps.iconPosition.default !== FIGMA_BUTTON_CONTRACT.properties.iconPosition.default) {
      result.errors.push(
        `iconPosition default value mismatch. Expected: "${FIGMA_BUTTON_CONTRACT.properties.iconPosition.default}", Got: "${componentProps.iconPosition.default}"`
      );
      result.valid = false;
    }

    // Validate allowed values
    const contractPositions = FIGMA_BUTTON_CONTRACT.properties.iconPosition.values as readonly string[];
    const componentPositions = componentProps._iconPositionValues || [];

    const missingPositions = contractPositions.filter((p: string) => !componentPositions.includes(p));
    const extraPositions = componentPositions.filter((p: string) => !contractPositions.includes(p));
  ...
  if (!componentProps.destructive) {
    ...
    if (componentProps.destructive.default !== expectedDefault) {
      result.errors.push(
        `destructive default value mismatch. Expected: ${expectedDefault}, Got: ${componentProps.destructive.default}`
      );
      result.valid = false;
    }
  }
