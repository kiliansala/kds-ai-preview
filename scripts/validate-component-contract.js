 ...
 */
function extractTypeValues(fileContent, typeName) {
 ...
 */
function validateComponent(componentPath, contract) {
    const result = {
        valid: true,
        errors: [],
        warnings: []
    ...
    for (const [propName, propSpec] of Object.entries(contract.properties)) {
        ...
        if (componentProp.default !== expectedDefault) {
            result.errors.push(`${propName} default value mismatch. Expected: "${expectedDefault}", Got: "${componentProp.default}"`);
            result.valid = false;
        }
        // Validate allowed values if specified
        if (propSpec.values && propSpec.values.length > 0) {
            const contractValues = propSpec.values;
            ...
            if (componentValues.length > 0) {
                const missingValues = contractValues.filter((v) => !componentValues.includes(v));
                const extraValues = componentValues.filter((v) => !contractValues.includes(v));
 ...
 */
function loadContract(componentName) {
...
// Run
main();
//# sourceMappingURL=validate-component-contract.js.map